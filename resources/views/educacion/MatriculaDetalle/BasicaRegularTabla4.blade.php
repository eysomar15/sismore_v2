<table id="tabla0" class="table table-striped table-bordered tablex" style="font-size:11px;">
    ...
    <tfoot>
        <tr class="text-center bg-primary text-white">
            ...
            <th>{{ number_format($item->ttp, 0) }}</th>
            <th>{{ number_format($item->ttph, 0) }}</th>
            <th>{{ number_format($item->ttpm, 0) }}</th>
            <th>{{ number_format($item->ICIII1H, 0) }}</th>
            <th>{{ number_format($item->ICIII1M, 0) }}</th>
            <th>{{ number_format($item->ICIII2H, 0) }}</th>
            <th>{{ number_format($item->ICIII2M, 0) }}</th>
            <th>{{ number_format($item->ICIV3H, 0) }}</th>
            <th>{{ number_format($item->ICIV3M, 0) }}</th>
            <th>{{ number_format($item->ICIV4H, 0) }}</th>
            <th>{{ number_format($item->ICIV4M, 0) }}</th>
            <th>{{ number_format($item->ICV5H, 0) }}</th>
            <th>{{ number_format($item->ICV5M, 0) }}</th>
            <th>{{ number_format($item->ICV6H, 0) }}</th>
            <th>{{ number_format($item->ICV6M, 0) }}</th>
        </tr>
    </tfoot>
</table>
